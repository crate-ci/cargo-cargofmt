...
fn strip_string_value(line: &str) -> &str {
    let mut equal_appears: bool = false;
...
"#]],
    ...
    fn no_overflow() {
        let input = "[package]\nname = \"app\"\n";
        assert_eq!(super::check_line_overflow(input, 20), vec![]);
    }

    #[test]
    fn overflow_table_header() {
        let input = "[very-long-table-header-name]\n";
        assert_eq!(super::check_line_overflow(input, 20), vec![(1, 29)]);
    }

    #[test]
    fn exempt_standalone_comment() {
        let input = "# This is a very long comment line\n";
        assert_eq!(super::check_line_overflow(input, 10), vec![]);
    }
    ...
    fn overflow_from_long_key() {
        ...
        assert_eq!(super::check_line_overflow(input, 10), vec![(1, 47)]);
    }
    #[test]
    ...
    fn not_exempt_hash_inside_string() {
        ...
        assert_eq!(super::check_line_overflow(input, 10), vec![(1, 27)]);
    }
}
